...
import { listTCQuestions, submitTCAnswer } from '@/lib/api';
import type { TCQuestion } from '@/lib/api';
...
export default function TCPage() {
  ...
  useEffect(() => {
    listTCQuestions({ limit: 20 })
      .then((qs) => {
        setQuestions(qs);
        if (qs.length > 0) setSelected(new Array(qs[0].blankCount).fill(''));
      })
      .catch(() => {})
  ...
  const handleSelect = (blankIndex: number, choice: string) => {
    if (submitted) return;
    setSelected((prev) => {
      const next = [...prev];
      next[blankIndex] = choice;
      return next;
    });
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await submitTCAnswer(question.id, selected);
      setResult(res);
      setSubmitted(true);
    } catch {
      // ignore
  ...
  const handleNext = () => {
    ...
    if (questions[nextIndex]) {
      setSelected(new Array(questions[nextIndex].blankCount).fill(''));
    }
  };

  const renderPrompt = (prompt: string) => {
    const parts = prompt.split(/\[BLANK_\d\]/);
    return parts.map((part, i) => {
      if (i === parts.length - 1) return <span key={i}>{part}</span>;
      return (
        <span key={i}>
          {part}
          <span className="inline-block border-b-2 border-[#1F2430] w-24 mx-1 translate-y-1" />
        </span>
      );
    });
  };
    ...
    return (
      <div className="max-w-4xl mx-auto text-center py-32">
        ...
        <p className="text-[#1F2430]/60 mb-8">
          Text Completion questions haven't been added yet.
        </p>
  ...
  const isAllAnswered = selected.every((s) => s !== '');
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500 pb-10">
      <header className="flex items-center justify-between mb-6">
        ...
        <div className="font-serif font-bold text-[#1F2430]">Text Completion</div>
      ...
      <div
        className={`grid gap-8 mt-12 ${
          question.blankCount === 1
            ? 'max-w-sm mx-auto'
            : question.blankCount === 2
              ? 'md:grid-cols-2'
              : 'md:grid-cols-3'
        }`}
      >
        {question.options.map((column, colIndex) => (
          <div key={colIndex} className="space-y-4">
            <h3 className="font-bold text-sm tracking-wider uppercase text-[#1F2430]/40 text-center mb-4">
              Blank {colIndex + 1}
            </h3>
            {column.map((choice) => {
              const isSelected = selected[colIndex] === choice;
              const isCorrectAnswer = result?.correctAnswers[colIndex] === choice;
              ...
              return (
                <button
                  key={choice}
                  onClick={() => handleSelect(colIndex, choice)}
                  disabled={submitted}
